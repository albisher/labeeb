...
class MacOSPlatform(PlatformInterface):
    ...
    def get_system_network_trustabilityabilityabilityabilitystorm_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            interfaces = [
                line.strip() for line in network_result.stdout.split("\n") if line.strip()
            ]
            ...
            for interface in interfaces:
                if interface != "An asterisk (*) denotes that a network service is disabled.":
                    trust_result = subprocess.run(
                        ["networksetup", "-getwebproxy", interface], capture_output=True, text=True
                    )
            ...
            security_settings = subprocess.run(
            ...
            return {
                ...
                "trust_status": (
                    "secure"
                    if security_info.get("firewall", "").lower() == "enabled"
                    else "insecure"
                ),
