...
class EmailTool(BaseTool):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        """
        super().__init__(
            name="email", description="Tool for performing email operations", config=config
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self._smtp_client = aiosmtplib.SMTP(
                hostname=self._smtp_host, port=self._smtp_port, use_tls=True
            )
        ...
        try:
            ...
            for attachment in attachments:
                if attachment.get("size", 0) > self._max_attachment_size:
                    return {
                        "error": f"Attachment too large (max: {self._max_attachment_size} bytes)"
                    }
            ...
            # Send email
            await self._smtp_client.send_message(message, to_addrs=to + cc + bcc)
            ...
            self._add_to_history(
                "send", {"to": to, "subject": subject, "attachment_count": len(attachments)}
            )
    ...
    async def _receive_emails(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._add_to_history("receive", {"folder": folder, "message_count": len(messages)})
    ...
    async def _search_emails(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._add_to_history(
                "search", {"query": query, "folder": folder, "message_count": len(messages)}
            )
    ...
    async def _delete_email(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._add_to_history("delete", {"message_id": message_id, "folder": folder})
