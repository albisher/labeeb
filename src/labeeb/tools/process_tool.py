...
class ProcessTool(BaseTool):
    ...
    def _start_process(
        self, command: str, args: Optional[List[str]] = None, cwd: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Start a new process."""
        try:
            ...
            self._operation_history.append(
                {
                    "operation": "start",
                    "details": {"pid": process.pid, "command": command, "args": args},
                    "timestamp": time.time(),
                }
            )
    ...
    def _stop_process(self, pid: int, force: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self._operation_history.append(
                {"operation": "stop", "details": {"pid": pid}, "timestamp": time.time()}
            )

            if len(self._operation_history) > self._max_history:
                self._operation_history.pop(0)

            return {"pid": pid, "stopped": True, "force": force}
    ...
    def _get_process_info(self, pid: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self._operation_history.append(
                {"operation": "status", "details": {"pid": pid}, "timestamp": time.time()}
            )
            ...
            return {
                ...
                "num_handles": (
                    system_process.num_handles() if hasattr(system_process, "num_handles") else None
                ),
                "io_counters": (
                    system_process.io_counters()._asdict() if system_process.io_counters() else None
                ),
                "connections": (
                    [conn._asdict() for conn in system_process.connections()]
                    if system_process.connections()
                    else None
                ),
                "process_info": status,
            }
