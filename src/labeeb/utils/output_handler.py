...
class OutputHandler:
    ...
    def __init__(self, theme: str = "default") -> None:
        ...
        # Core styling engine
        self.style_mgr = OutputStyleManager(
            theme=theme,
            config_path=str(Path(__file__).parent.parent.parent / "config" / "output_styles.json"),
        )
    ...
    def thinking(self, message: str) -> Optional[str]:
        ...
        formatted_box = self.style_mgr.format_box(
            content=message, title="Thinking" if not self.rtl_support else "جاري التفكير"
        )
    ...
    def status(self, message: str, status_key: str = "info") -> str:
        ...
        return self.capture_print(self.style_mgr.format_status_line(message, status_key))
    ...
    def header(self, text: str, emoji_key: Optional[str] = None) -> str:
        ...
        return self.capture_print(self.style_mgr.format_header(text, emoji_key))
    ...
    def box(self, content: str, title: Optional[str] = None, width: Optional[int] = None) -> str:
        ...
        return self.capture_print(self.style_mgr.format_box(content, title, width))
    ...
    def divider(self, style: str = "partial") -> str:
        ...
        """
        return self.capture_print(self.style_mgr.create_divider(style))

    def table(self, headers: List[str], rows: List[List[Any]], title: Optional[str] = None) -> str:
        """
        ...
        return self.capture_print(self.style_mgr.format_table(headers, rows, title))

    def list_items(
        self, items: List[str], bullet_type: str = "symbol", title: Optional[str] = None
    ) -> str:
        """
        ...
        return self.capture_print(self.style_mgr.format_list(items, bullet_type, title))
