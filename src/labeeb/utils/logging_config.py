...
class LoggingConfig:
    ...
    def _configure_windows_logging(self) -> None:
        ...
        try:
            ...
            windows_log_path = os.path.join(
                self.platform_info["paths"]["logs"], "windows_specific.log"
            )
            windows_handler = logging.FileHandler(windows_log_path)
            windows_handler.setFormatter(
                logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
            )
    ...
    def _configure_ubuntu_logging(self) -> None:
        ...
        try:
            ...
            ubuntu_log_path = os.path.join(
                self.platform_info["paths"]["logs"], "ubuntu_specific.log"
            )
            ubuntu_handler = logging.FileHandler(ubuntu_log_path)
            ubuntu_handler.setFormatter(
                logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
            )
    ...
    def get_platform_log_path(self) -> str:
        ...
        return os.path.join(
            self.platform_info["paths"]["logs"], f"{self.platform_info['name']}_specific.log"
        )
