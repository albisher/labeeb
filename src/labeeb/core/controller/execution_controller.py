...
class ExecutionController:
    ...
    def execute_plan(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "success" if not self.execution_state["failed_steps"] else "partial_success"
                ),
                "results": results,
    ...
    def _execute_operation(self, operation: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if op_type in ["launch_application", "open_application", "run_application"]:
                app_name = parameters.get("application", "") or parameters.get(
                    "application_name", ""
                )
                ...
                if (
                    not os.path.exists(f"/Applications/{app_name_mapped}.app")
                    and " " in app_name_mapped
                ):
                ...
                if mode == "private":
                    ...
                    elif platform_utils.is_posix() and os.path.exists("/Applications"):
            ...
            if op_type == "system_check":
                return (
                    f"Stub: Operation 'system_check' is not implemented. Parameters: {parameters}"
                )
            if op_type == "system_lookup":
                return (
                    f"Stub: Operation 'system_lookup' is not implemented. Parameters: {parameters}"
                )
            ...
            if op_type == "calendar_navigation":
                ...
                if platform_utils.is_posix() and os.path.exists("/Applications/Calendar.app"):
            ...
            if op_type == "application_list":
                ...
                if platform_utils.is_posix() and os.path.exists("/Applications"):
                    try:
    ...
    def _handle_application_operation(self, operation: str, parameters: Dict[str, Any]) -> Any:
        ...
        if operation == "application.launch":
            ...
            if platform_utils.is_windows():  # Windows
                cmd = f'start "" "{app_name}"'
            elif platform_utils.is_posix():  # macOS and Linux
