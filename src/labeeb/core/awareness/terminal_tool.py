...
style_mgr = OutputStyleManager(
    config_path=str(Path(__file__).parent.parent.parent.parent / "config" / "output_styles.json")
)
...
class TerminalTool:
    ...
    async def execute(self, action: str, **kwargs) -> Any:
        ...
        try:
            actions = {
                "show_text": lambda: self.show_text(
                    kwargs.get("text", ""), kwargs.get("color"), kwargs.get("style")
                ),
