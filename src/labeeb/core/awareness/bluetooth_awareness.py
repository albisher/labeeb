        ...
        try:
            if is_mac():
                try:
                    ...
                    raise ImportError(
                        "pyobjc IOBluetooth not fully implemented, falling back to generic."
                    )
                except Exception as e:
                    logger.info(
                        f"pyobjc IOBluetooth not available or failed: {e}. Falling back to generic."
                    )
        ...
        try:
            ...
            for line in out.splitlines():
                if line.startswith("Device"):
                    ...
                    device = BluetoothDevice(
                        name=name, address=addr, connected=True, status="connected"
                    )
                    self.devices.append(device)
        ...
        try:
            ...
            for line in out.splitlines():
                if "Address" in line:
                    ...
                    self.devices.append(
                        BluetoothDevice(
                            name="Unknown", address=address, connected=False, status="unknown"
                        )
                    )
        except Exception as e:
    ...
    def get_bluetooth_devices(self) -> Dict[str, Any]:
        ...
        if self.devices:
            ...
            return {
                "devices": [],
                "status": "unavailable",
                "message": "No Bluetooth devices found. Ensure Bluetooth is enabled and devices are paired.",
            }
    ...
    def get_connected_devices(self) -> Dict[str, Any]:
        ...
        if connected:
            ...
            return {
                "devices": [],
                "status": "unavailable",
                "message": "No connected Bluetooth devices found.",
            }
