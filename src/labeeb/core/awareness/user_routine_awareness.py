...
class UserRoutineAwarenessTool:
    ...
    async def get_user_routine(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "screen_dim_idle_seconds": (
                (now - self.activity["last_screen_dim_time"])
                if self.activity["last_screen_dim_time"]
                else None
            ),
            "keyboard_idle_seconds": (
                (now - self.activity["last_keyboard_activity"])
                if self.activity["last_keyboard_activity"]
                else None
            ),
            "mouse_idle_seconds": (
                (now - self.activity["last_mouse_activity"])
                if self.activity["last_mouse_activity"]
                else None
            ),
            "window_change_idle_seconds": (
                (now - self.activity["last_window_change"])
                if self.activity["last_window_change"]
                else None
            ),
    ...
    def get_user_routines(self) -> Dict[str, Any]:
        ...
        try:
            routines = {"platform": self.platform_info["name"], "routines": {}}
    ...
    def _get_default_routines(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "shortcuts": {"system": ["Alt+Tab", "Alt+F4"], "apps": ["Ctrl+N", "Ctrl+S", "Ctrl+W"]},
        }
