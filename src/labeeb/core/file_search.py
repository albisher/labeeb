...
class FileSearch:
    ...
    def _get_notes_folders(self, cloud_folders: List[Dict[str, str]]) -> None:
        ...
        if self.system_platform == "darwin":
            ...
            icloud_path = get_file_path(
                os.path.expanduser("~/Library/Mobile Documents/com~apple~Notes")
            )
    ...
    def _find_folders_natively(
        self, folder_name: str, location: str, max_results: int, result_list: List[str]
    ) -> None:
        """
        ...
        self._search_directory(
            location, folder_name, max_depth, 0, result_list, max_results, is_wildcard
        )
        ...
        try:
            ...
            if not is_wildcard:
                if pattern.lower() in [item.lower() for item in items]:
                    for item in items:
                        if item.lower() == pattern.lower() and os.path.isdir(
    ...
    def _search_files_directory(
        self,
        directory: str,
        file_pattern: str,
        pattern_obj: Optional[re.Pattern],
        result_list: List[str],
        max_results: int,
        current_depth: int,
        max_depth: int,
