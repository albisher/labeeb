        ...
        try:
            if self.system == "darwin":  # macOS
                    ...
                    for section in sections:
                        if "Serial Number" in section or "Product ID" in section:
                            ...
                            if product_match:
                                ...
                                vendor = (
                                    vendor_match.group(1).strip() if vendor_match else "Unknown"
                                )
                                serial = (
                                    f" (S/N: {serial_match.group(1).strip()})"
                                    if serial_match
                                    else ""
                                )
                ...
                if tty_result.returncode == 0:
                    tty_devices = [
                        dev
                        for dev in tty_result.stdout.strip().split("\n")
                ...
                try:
                    ...
                    if lsusb_result.returncode == 0 and lsusb_result.stdout.strip():
                        devices.append("\nUSB Device Information:")
                        devices.extend(
                            [f"  - {line}" for line in lsusb_result.stdout.strip().split("\n")]
                        )
                except Exception:
                    pass
                ...
                try:
                    ...
                    if dmesg_result.returncode == 0 and dmesg_result.stdout.strip():
                        ...
                        devices.extend(
                            [f"  - {line}" for line in dmesg_result.stdout.strip().split("\n")]
                        )
                except Exception:
                    pass
