...
class CommandPatternDetector:
    ...
    def extract_screen_command(self, query_lower):
        ...
        for i in range(len(words) - 3):
            if (
                words[i] in ["make", "tell", "have"]
                and words[i + 1] in ["it", "screen", "terminal", "session"]
                and words[i + 2] in ["do", "run", "execute", "type"]
            ):
        ...
        for i in range(len(words) - 3):
            if (
                words[i] in ["in", "on", "at"]
                and words[i + 1] in ["the", "screen", "terminal", "session"]
                and any(words[i + j] in ["do", "run", "execute", "type"] for j in range(2, 4))
            ):
                # Find the verb
        ...
        for i in range(len(words) - 3):
            if words[i] in ["send", "run", "do", "execute", "type"]:
                ...
                if command:
                    ...
                    if any(
                        indicator in remaining_after_command
                        for indicator in ["to", "in", "on", "at", "screen", "terminal", "session"]
                    ):
                        return command

        # If no complex pattern matched, check for simple commands
        for cmd in self.common_commands:
            if cmd in query_lower and any(
                term in query_lower for term in ["screen", "terminal", "session", "serial"]
            ):
                return cmd
    ...
    def extract_device_command(self, query_lower):
        ...
        for i in range(len(words) - 3):
            if (
                words[i] in ["on", "to", "with"]
                and words[i + 1] in ["the", "usb", "serial", "device", "remote", "screen"]
                and any(
                    words[i + j] in ["do", "run", "execute", "type", "send"] for j in range(2, 4)
                )
            ):
                # Find the verb
                verb_idx = (
                    i + 2 if words[i + 2] in ["do", "run", "execute", "type", "send"] else i + 3
                )
                # Extract command after the verb
                remaining = " ".join(words[verb_idx + 1 :])
                return self._extract_quoted_command(remaining)
