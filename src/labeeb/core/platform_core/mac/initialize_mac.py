...
def check_macos():
    ...
    if platform.machine() not in ["arm64", "aarch64"]:
        print(
            f"Warning: This script is optimized for Apple Silicon, but detected {platform.machine()}"
        )
...
def install_dependencies():
    ...
    if not os.path.exists("/opt/homebrew/bin/brew") and not os.path.exists("/usr/local/bin/brew"):
        ...
        # Check if successful
        if not os.path.exists("/opt/homebrew/bin/brew") and not os.path.exists(
            "/usr/local/bin/brew"
        ):
...
def setup_ollama():
    ...
    if not os.path.exists("/opt/homebrew/bin/ollama") and not os.path.exists(
    ...
    ollama_path = (
        ...
        if os.path.exists("/opt/homebrew/bin/ollama")
...
def setup_platform_specific():
    ...
    audio_dir = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "audio", "recordings"
    )
    ...
    platform_test_script = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "test_platform_components.py"
    )
