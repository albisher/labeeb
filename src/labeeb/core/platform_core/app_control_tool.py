            ...
            if self.platform == "darwin":
                subprocess.run(
                    [
                        "osascript",
                        "-e",
                        f'tell application "{app_name}" to set visible of window 1 to false',
                    ]
                )
            elif self.platform == "win32":
                subprocess.run(
                    [
                        "powershell",
                        "-command",
                        f'(New-Object -ComObject WScript.Shell).AppActivate("{app_name}"); (New-Object -ComObject WScript.Shell).SendKeys("% n")',
                    ]
                )
            else:
                return {"error": f"Unsupported platform: {self.platform}"}
            ...
            if self.platform == "darwin":
                subprocess.run(
                    [
                        "osascript",
                        "-e",
                        f'tell application "{app_name}" to set visible of window 1 to true',
                    ]
                )
            elif self.platform == "win32":
                subprocess.run(
                    [
                        "powershell",
                        "-command",
                        f'(New-Object -ComObject WScript.Shell).AppActivate("{app_name}"); (New-Object -ComObject WScript.Shell).SendKeys("% x")',
                    ]
                )
            else:
                return {"error": f"Unsupported platform: {self.platform}"}
