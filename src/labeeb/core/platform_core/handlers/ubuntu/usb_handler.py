...
class UbuntuUSBHandler:
        ...
        if self._has_pyudev:
            ...
            try:
                ...
                result = subprocess.run(
                    ["lsblk", "-J", "-o", "NAME,SIZE,TYPE,MOUNTPOINT,VENDOR,MODEL"],
                    capture_output=True,
                    text=True,
                )
    ...
    def _get_mount_point(self, device_node):
        ...
        try:
            ...
            result = subprocess.run(
                ["findmnt", "-n", "-o", "TARGET", device_node], capture_output=True, text=True
            )
            if result.returncode == 0:
    ...
    def _get_device_size(self, device_node):
        ...
        try:
            ...
            result = subprocess.run(
                ["blockdev", "--getsize64", device_node], capture_output=True, text=True
            )
            if result.returncode == 0:
