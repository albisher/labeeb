    ...
    def get_content(self, browser_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            if not browser_name or browser_name == "safari":
                ...
                """
                result = run_command(
                    ["osascript", "-e", safari_script], capture_output=True, text=True
                )
            ...
            if not browser_name or browser_name in ["chrome", "google chrome"]:
                ...
                """
                result = run_command(
                    ["osascript", "-e", chrome_script], capture_output=True, text=True
                )
                if result["success"] and result["stdout"].strip():
            ...
            if not browser_name or browser_name in ["firefox", "mozilla firefox"]:
                ...
                """
                result = run_command(
                    ["osascript", "-e", firefox_script], capture_output=True, text=True
                )
                if result["success"] and result["stdout"].strip():
