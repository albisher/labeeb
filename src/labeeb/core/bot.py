...
class Labeeb:
    ...
    def process_command(self, command: str) -> Dict[str, Any]:
        ...
        for cap_name in required_caps:
            ...
            if not status["is_ready"]:
                missing_caps.append(
                    {"name": cap_name, "missing_dependencies": status["missing_dependencies"]}
                )
    ...
    def update_capability_status(
        self, name: str, is_tested: bool, test_coverage: float, status: Optional[str] = None
    ) -> None:
        ...
        self.capabilities.update_test_status(
            name=name, is_tested=is_tested, test_coverage=test_coverage, status=status
        )
