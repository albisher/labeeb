...
def parse_file_request(request: str) -> Dict[str, Any]:
    ...
    if not content:
        content_match = re.search(
            r"(?:content|with|containing)\s+[\'\"]([^\'\"]+)[\'\"]", request, re.IGNORECASE
        )
        if content_match:
    ...
    if not filename or not content or not result.get("operation"):
        arabic_match = re.search(
            r"إنشاء\s+ملف\s+([\w\.]+)\s+في\s+المجلد\s+([\w/]+)\s+بالمحتوى\s+'([^']+)'", request
        )
        if arabic_match:
            filename = arabic_match.group(1)
    ...
    eng_match = re.search(
        r"create\s+file\s+([^\s'\"]+)\s+in\s+folder\s+([^\s'\"]+)\s+with\s+content\s+'([^']+)'",
        request,
        re.IGNORECASE,
    )
    if eng_match:
    ...
    eng_match2 = re.search(
        r"create\s+file\s+([\w\.]+)\s+with\s+content\s+'([^']+)'", request, re.IGNORECASE
    )
    if eng_match2:
