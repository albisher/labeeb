    ...
    def __init__(
        ...
        if self._platform_handler is None:
            raise NotImplementedError(
                f"No shell handler available for platform {platform_manager.get_platform()}"
            )
        ...
        self._log_debug(
            f"ShellHandler initialized. Safe mode: {safe_mode}, Dangerous command check: {enable_dangerous_command_check}, Fast mode: {fast_mode}"
        )
