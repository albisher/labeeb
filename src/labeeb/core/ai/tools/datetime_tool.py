...
class DateTimeTool(BaseTool):
    ...
    def _format_datetime(
        self, dt: datetime, format_str: str = "%Y-%m-%d %H:%M:%S", **kwargs
    ) -> Dict[str, Any]:
        """Format a datetime object."""
        return {"formatted": dt.strftime(format_str)}

    def _parse_datetime(
        self, dt_str: str, format_str: str = "%Y-%m-%d %H:%M:%S", **kwargs
    ) -> Dict[str, Any]:
        """Parse a datetime string."""
        dt = datetime.strptime(dt_str, format_str)
        return {"datetime": dt.isoformat(), "timestamp": dt.timestamp()}

    def _add_time(
        self, dt: datetime, days: int = 0, hours: int = 0, minutes: int = 0, **kwargs
    ) -> Dict[str, Any]:
