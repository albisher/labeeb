"""
Config tool with A2A, MCP (Model Context Protocol), and SmolAgents compliance.
...
class ConfigTool(BaseTool):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        """
        super().__init__(
            name="config", description="Tool for performing configuration operations", config=config
        )
        ...
        try:
            ...
            if key:
                if key not in config:
                    return {"status": "error", "action": "get", "error": f"Key not found: {key}"}
                value = config[key]
            else:
                value = config

            self._add_to_history("get", {"name": name, "key": key, "format": format})

            return {"status": "success", "action": "get", "name": name, "key": key, "value": value}
        except Exception as e:
    ...
    async def _set(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._add_to_history("set", {"name": name, "key": key, "format": format})

            return {"status": "success", "action": "set", "name": name, "key": key}
        except Exception as e:
    ...
    async def _delete(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if key:
                if key not in config:
                    return {"status": "error", "action": "delete", "error": f"Key not found: {key}"}
                del config[key]
            else:
                config = {}
            ...
            self._add_to_history("delete", {"name": name, "key": key, "format": format})

            return {"status": "success", "action": "delete", "name": name, "key": key}
        except Exception as e:
    ...
    async def _list(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._add_to_history("list", {"configs": len(configs)})

            return {"status": "success", "action": "list", "configs": configs}
        except Exception as e:
