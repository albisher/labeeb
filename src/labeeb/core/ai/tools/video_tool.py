...
class VideoTool(BaseTool):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        """
        super().__init__(
            name="video", description="Tool for performing video operations", config=config
        )
            ...
            with io.BytesIO(video_data) as temp_file:
                ...
                if operation == "convert":
                    ...
                    for i in range(segments):
                        ...
                        end_frame = (
                            start_frame + segment_frames if i < segments - 1 else frame_count
                        )
                        cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame)
                        for _ in range(end_frame - start_frame):
                            ret, frame = cap.read()
    ...
    async def _convert_format(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(args["video_data"], "convert", format=args["format"])

            if "error" not in result:
                self._add_to_history("convert", {"format": args["format"], "size": result["size"]})
    ...
    async def _trim_video(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(
                args["video_data"], "trim", start=args.get("start", 0), end=args.get("end")
            )

            if "error" not in result:
                self._add_to_history(
                    "trim",
                    {"start": args.get("start", 0), "end": args.get("end"), "size": result["size"]},
                )
    ...
    async def _merge_video(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(
                args["video_data"], "merge", other_video=args["other_video"]
            )

            if "error" not in result:
                self._add_to_history("merge", {"size": result["size"]})
    ...
    async def _split_video(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(
                args["video_data"], "split", segments=args.get("segments", 2)
            )
    ...
    async def _resize_video(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(
                args["video_data"], "resize", width=args.get("width"), height=args.get("height")
            )
    ...
    async def _rotate_video(self, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._process_video(
                args["video_data"], "rotate", angle=args.get("angle", 0)
            )

            if "error" not in result:
                self._add_to_history(
                    "rotate", {"angle": args.get("angle", 0), "size": result["size"]}
                )
