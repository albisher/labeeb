    ...
    def _setup_translations(self, language_code: str) -> None:
        ...
        try:
            ...
            self.translations = gettext.translation(
                "labeeb", localedir="locales", languages=[language_code], fallback=True
            )
    ...
    async def locate_on_screen(self, image_path: str, confidence: float = 0.9) -> Dict[str, Any]:
        ...
        try:
            self._log_protocol_action(
                "A2A", "locate_on_screen", {"image_path": image_path, "confidence": confidence}
            )
            self._log_protocol_action(
                "MCP", "locate_on_screen", {"image_path": image_path, "confidence": confidence}
            )
            platform_name = (
                self.platform_info.get("name") or self.platform_info.get("platform") or "unknown"
            )
