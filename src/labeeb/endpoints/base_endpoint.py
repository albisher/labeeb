...
---
description: Base class for all endpoints in the system
...
class BaseEndpoint(ABC):
    """Base class for all endpoints in the system."""
    
    def __init__(self):
        """Initialize the base endpoint."""
        self.name = "base_endpoint"
        self.description = "Base class for all endpoints"
        self.version = "1.0.0"
    ...
    def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing the result of executing the endpoint
        """
        try:
            if not self.validate_config():
                return {
                    "status": "error",
                    "message": "Endpoint configuration is invalid"
                }
            ...
            # Execute endpoint
            result = self._execute_endpoint(input_data)
            ...
            return {
                "status": "error",
                "message": f"Failed to execute endpoint: {str(e)}"
            }
    
    @abstractmethod
    def _execute_endpoint(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """Execute the endpoint.
